...
- **Decoded paths are re-escaped.** HTTP APIs deliver the request path url-decoded (payload format 2.0 `rawPath` included), while REST APIs and Lambda Function URLs deliver it still encoded. The adapter only escapes characters that can never appear in an encoded path (`?`, `#`, spaces, non-ascii, a `%` that does not start an escape, ...), so encoded paths pass through unchanged and a decoded `?` or `#` cannot be mistaken for the query string or fragment. Characters API Gateway already decoded into url syntax (`%2F` → `/`) or dropped cannot be recovered.
